...
import com.shapesecurity.shift.es2018.parser.token.StringLiteralToken;
...
public class Validator extends MonoidalReducer<ValidationContext> {
    ...
    @Nonnull
    @Override
    public ValidationContext reduceForAwaitStatement(@Nonnull ForAwaitStatement node, @Nonnull ValidationContext left, @Nonnull ValidationContext right, @Nonnull ValidationContext body) {
        ValidationContext s = super.reduceForAwaitStatement(node, left, right, body);
        if (node.left instanceof VariableDeclaration) {
            VariableDeclaration varDec = (VariableDeclaration) node.left;
            if (varDec.declarators.length != 1) {
                s.addError(new ValidationError(node, ValidationErrorMessages.ONE_VARIABLE_DECLARATOR_IN_FOR_AWAIT));
            }
            if (varDec.declarators.maybeHead().fromJust().init.isJust()) {
                s.addError(new ValidationError(node, ValidationErrorMessages.NO_INIT_IN_VARIABLE_DECLARATOR_IN_FOR_AWAIT));
            }
        }
        return s;
    }
