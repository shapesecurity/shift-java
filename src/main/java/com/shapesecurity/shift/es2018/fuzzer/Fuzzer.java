...
package com.shapesecurity.shift.es2018.fuzzer;
...
public class Fuzzer {
    ...
    private final static Gen<IterationStatement>[] iterationStatementGens = Fuzzer.<Gen<IterationStatement>>array(
            ...
            Fuzzer::randomForAwaitStatement,
...
//        }
    ...
    @Nonnull
    private static ForAwaitStatement randomForAwaitStatement(@Nonnull GenCtx ctx, int depth) {
        return new ForAwaitStatement(randomVariableDeclarationAssignmentTarget(ctx.inForInOfStatement(), depth - 1), randomExpression(ctx, depth - 1), randomStatement(ctx, depth - 1));
    }
    ...
    private static LiteralRegExpExpression randomLiteralRegExpExpression(@Nonnull GenCtx ctx, int depth) {
        return new LiteralRegExpExpression(randomRegExpString(ctx, depth - 1), ctx.random.nextBoolean(), ctx.random.nextBoolean(), ctx.random.nextBoolean(), ctx.random.nextBoolean(), ctx.random.nextBoolean(), ctx.random.nextBoolean());
    }
    ...
    private static ObjectProperty randomObjectProperty(@Nonnull GenCtx ctx, int depth) {
        switch (ctx.random.nextInt(6)) {
            ...
            case 2:
                return randomSpreadProperty(ctx, depth);
            default:
    ...
    @Nonnull
    private static SpreadProperty randomSpreadProperty(@Nonnull GenCtx ctx, int depth) {
        return new SpreadProperty(randomExpression(ctx, depth - 1));
    }
