...
import com.shapesecurity.shift.es2018.utils.D2A;
import com.shapesecurity.shift.es2018.utils.Either3;
...
public abstract class GenericParser<AdditionalStateT> extends Tokenizer {
    ...
    protected Binding parseObjectBinding() throws JsError {
        ...
        while (!this.match(TokenType.RBRACE)) {
            if (this.eat(TokenType.ELLIPSIS)) {
                rest = Maybe.of(this.parseBindingIdentifier());
                break;
            }
        ...
        return this.finishNode(startState, new ObjectBinding(ImmutableList.from(properties), rest));
    }
    ...
    @Nonnull
    protected Expression parseFunctionExpression(boolean isAsync) throws JsError {
        AdditionalStateT startState = this.startNode();
        this.lex();
    ...
    protected Statement parseForStatement() throws JsError {
        this.lex();
        boolean await = this.allowAwaitExpression && this.eat(TokenType.AWAIT);

        this.expect(TokenType.LPAREN);
        Maybe<Expression> test = Maybe.empty();
        Maybe<Expression> right = Maybe.empty();
        if (await && this.match(TokenType.SEMICOLON)) {
            throw this.createUnexpected(this.lookahead);
        }
        if (this.eat(TokenType.SEMICOLON)) {
        ...
        if (this.match(TokenType.VAR) || isForDecl) {
            ...
            if (init.declarators.length == 1 && (this.match((TokenType.IN)) || this.matchContextualKeyword("of"))) {
                ...
                if (declarator.init.isJust()) {
                    throw this.createError(await ? ErrorMessages.INVALID_VAR_INIT_FOR_AWAIT : ErrorMessages.INVALID_VAR_INIT_FOR_OF);
                }
                this.lex();
        ...
        if (this.isAssignmentTarget && fromParseAssignmentOrTarget.left().map(expr -> !(expr instanceof AssignmentExpression)).orJust(true) && (this.match(TokenType.IN) || this.matchContextualKeyword("of"))) {
            ...
            if (startsWithLet && this.matchContextualKeyword("of")) {
                throw this.createError(await ? ErrorMessages.INVALID_LHS_IN_FOR_AWAIT : ErrorMessages.INVALID_LHS_IN_FOR_OF);
            }
            if (this.match(TokenType.IN)) {
                if (await) {
                    throw this.createUnexpected(this.lookahead);
    ...
    protected AssignmentTarget transformDestructuring(Expression node) throws JsError {
        if (node instanceof ObjectExpression) {
            ...
            if (objectExpression.properties.length > 0) {
                ObjectProperty last = objectExpression.properties.maybeLast().fromJust();
            ...
            for (ObjectProperty p : objectExpression.properties) {
                if (propertyIndex == objectExpression.properties.length - 1 && rest.isJust()) {
                    continue;
                }
                properties.add(this.transformDestructuring(p)); // TO BE DISCUSSED
                ++propertyIndex;
            }
            if (rest.isJust()) {
    ...
    protected Either3<Expression, Pair<FormalParameters, Boolean>, AssignmentTarget> parsePrimaryExpression() throws JsError {
        ...
        if (this.eat(TokenType.ASYNC)) {
            if (!this.hasLineTerminatorBeforeNext && this.match(TokenType.FUNCTION)) {
                this.isBindingElement = this.isAssignmentTarget = false;
                return Either3.left(this.finishNode(startState, this.parseFunctionExpression(true)));
            }
        ...
        switch (this.lookahead.type) {
            ...
            case FUNCTION:
                this.isBindingElement = this.isAssignmentTarget = false;
                return Either3.left(this.finishNode(startState, this.parseFunctionExpression(false)));
            case NUMBER:
                this.isBindingElement = this.isAssignmentTarget = false;
            ...
            case ASSIGN_DIV:
                ...
                boolean gFlag = false, iFlag = false, mFlag = false, uFlag = false, yFlag = false, sFlag = false;
                for (char c : flags.toCharArray()) {
                    switch (c) {
                        ...
                        case 's':
                            if (sFlag) {
                                throw this.createErrorWithLocation(this.getLocation(), "Duplicate regular expression flag 's'");
                            }
                            sFlag = true;
                            break;
                        default:
                ...
                return Either3.left(this.finishNode(startState, new LiteralRegExpExpression(pattern, gFlag, iFlag, mFlag, sFlag, uFlag, yFlag)));
            default:
                throw this.createUnexpected(this.lookahead);
        }
    }
    ...
    protected Either<Expression, AssignmentTarget> parseObjectExpression() throws JsError {
        ...
        while (!this.match(TokenType.RBRACE)) {
            Either<ObjectProperty, AssignmentTargetProperty> fromParsePropertyDefinition;
            boolean matchedSpreadProperty = false;
            if (this.match(TokenType.ELLIPSIS)) {
                ...
                if (spreadProperty.isLeft()) {
                    ...
                    rest = spreadProperty.right();
                    break;
                }
            } else {
                fromParsePropertyDefinition = this.parsePropertyDefinition();
            }
            ...
            if (allExpressionsSoFar) {
                ...
                if (fromParsePropertyDefinition.isLeft()) {
                    ObjectProperty property = fromParsePropertyDefinition.left().fromJust();
                    bindingProperties.add(this.transformDestructuring(property));
                } else {
                    bindingProperties.add(fromParsePropertyDefinition.right().fromJust());
                }
            }
    ...
    protected Binding targetToBinding(@Nonnull AssignmentTarget target) throws JsError {
        if (target instanceof AssignmentTargetIdentifier) {
        ...
        } else if (target instanceof ObjectAssignmentTarget) {
            ...
            return this.copyNode(target, new ObjectBinding(ImmutableList.from(properties), oat.rest.map(assignmentTarget -> {
                try {
                    return this.targetToBinding(assignmentTarget);
                } catch (JsError e) {
    ...
    protected Either<SpreadProperty, AssignmentTarget> parseSpreadPropertyDefinition() throws JsError {
        AdditionalStateT startState = this.startNode();
        SourceLocation startLocation = this.getLocation();

        this.expect(TokenType.ELLIPSIS);
        Either<Expression, AssignmentTarget> expressionOrAssignmentTarget = this.parseAssignmentExpressionOrTarget();
        if (!isValidSimpleAssignmentTarget(expressionOrAssignmentTarget.left().fromJust())) {
            this.isBindingElement = this.isAssignmentTarget = false;
        } else if (!(expressionOrAssignmentTarget.left().fromJust() instanceof IdentifierExpression))  {
            this.isBindingElement = false;
        }
        return expressionOrAssignmentTarget.mapLeft(expression -> this.finishNode(startState, new SpreadProperty(expression)));
    }
    ...
    protected Either<PropertyName, MethodDefinition> parseMethodDefinition() throws JsError {
        ...
        TokenizerState preAsyncTokenState = this.saveTokenizerState();

        boolean isAsync = this.eat(TokenType.ASYNC);

        if (isAsync && this.hasLineTerminatorBeforeNext) {
            isAsync = false;
            this.restoreTokenizerState(preAsyncTokenState);
        }
