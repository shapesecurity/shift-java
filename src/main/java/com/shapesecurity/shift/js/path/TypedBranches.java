...
public class TypedBranches {
  ...
  private static <P extends Node, C extends Node> TypedBranch<P, C> def(
    ...
    return new TypedBranch<>(parentType, childType, view, set);
  }

  private static <P extends Node, C> TypedBranch<P, ListNode<C>> defl(
      @Nonnull GenType parentType,
      @Nonnull GenType childType,
      @Nonnull F<P, List<C>> view,
      @Nonnull F2<P, List<C>, P> set) {
    return new TypedBranch<>(parentType, ListType.from(childType), list(view, childType), list(set));
  }

  private static <P extends Node, C> TypedBranch<P, NonEmptyListNode<C>> defnel(
      @Nonnull GenType parentType,
      @Nonnull GenType childType,
      @Nonnull F<P, NonEmptyList<C>> view,
      @Nonnull F2<P, NonEmptyList<C>, P> set) {
    return new TypedBranch<>(parentType, NonEmptyListType.from(childType), nel(view, childType), nel(set));
  }
  ...
  private static <P extends Node, C> TypedBranch<P, MaybeNode<C>> defm(
    ...
    return new TypedBranch<>(parentType, MaybeType.from(childType), maybe(view, childType), maybe(set));
  }
  ...
  private static <P extends Node, A, B> TypedBranch<P, EitherNode<A, B>> defe(
    ...
    return new TypedBranch<>(parentType, EitherType.from(a, b), either(view, a, b), either(set));
  }
