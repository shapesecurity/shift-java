...
import com.shapesecurity.shift.functional.Effect;
...
public abstract class HashTable<K, V> {
  ...
  public abstract void foreach(@Nonnull Effect<Pair<K, V>> e);
  ...
  private final static class Empty<K, V> extends HashTable<K, V> {
    ...
    @Override
    public void foreach(@Nonnull Effect<Pair<K, V>> e) {
  ...
  private final static class Leaf<K, V> extends HashTable<K, V> {
    ...
    protected Leaf(@Nonnull Hasher<K> hasher, @Nonnull List<Pair<K, V>> dataList, int baseHash, int length) {
      super(hasher, length);
      ...
      if (tree instanceof Empty) {
        ...
        if (leaf.baseHash == this.baseHash) {
          ...
          List<Pair<K, V>> right = leaf.dataList.foldLeft(
              (@Nonnull List<Pair<K, V>> result, @Nonnull Pair<K, V> kvPair) -> {
