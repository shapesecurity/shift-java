...
public abstract class GenericParser<AdditionalStateT> extends Tokenizer {
    ...
    protected boolean lookaheadLexicalDeclaration() throws JsError {
        if (this.match(TokenType.LET) || this.match(TokenType.CONST)) {
            ...
            this.lex();
            if (this.match(TokenType.IDENTIFIER) || this.match(TokenType.LET) || this.match(TokenType.LBRACE) || this.match(TokenType.LBRACK) || this.match(TokenType.YIELD) || this.match(TokenType.AWAIT)) {
    ...
    @Nonnull
    protected Statement parseAsync(boolean inDefault) throws JsError {
        AdditionalStateT startState = this.startNode();
        this.eat(TokenType.ASYNC);
        return this.parseFunctionDeclaration(startState, inDefault, true, false);
    }
    ...
    protected Statement parseStatementListItem() throws JsError {
        ...
        switch (this.lookahead.type) {
            case ASYNC:
                return this.parseAsync(false);
            case FUNCTION:
                return this.parseFunctionDeclaration(this.startNode(), false, false, true);
            case CLASS:
    ...
    protected BindingProperty parseBindingProperty() throws JsError {
        ...
        if ((token.type == TokenType.IDENTIFIER || token.type == TokenType.LET || token.type == TokenType.YIELD || token.type == TokenType.AWAIT) && name instanceof StaticPropertyName) {
            if (!this.match(TokenType.COLON)) {
                Maybe<Expression> defaultValue = Maybe.empty();
                if (this.eat(TokenType.ASSIGN)) {
                    boolean previousAllowYieldExpression = this.allowYieldExpression;
                    boolean previousAllowAwaitExpression = this.allowAwaitExpression;
                    ...
                    this.allowAwaitExpression = previousAllowAwaitExpression;
                } else if (token.type == TokenType.YIELD && this.allowYieldExpression) {
                    throw this.createUnexpected(token);
                } else if (token.type == TokenType.AWAIT && this.allowAwaitExpression) {
                    throw this.createUnexpected(token);
                }
    ...
    protected String parseIdentifier() throws JsError {
        if (this.match(TokenType.IDENTIFIER) || !this.allowAwaitExpression && this.match(TokenType.AWAIT) || !this.allowYieldExpression && this.match(TokenType.YIELD) || this.match(TokenType.LET)) {
    ...
    @Nonnull
    protected Expression parseArrowExpressionTail(boolean isAsync, FormalParameters paramsNode, AdditionalStateT startState) throws JsError {
        if (this.hasLineTerminatorBeforeNext) {
    ...
    protected Statement parseIfStatementChild() throws JsError {
        ...
        return this.match(TokenType.FUNCTION) ? this.parseFunctionDeclaration(startState, false, isAsync, false) : this.parseStatement();
    }

    @Nonnull
    protected Statement parseFunctionDeclaration(AdditionalStateT startState, boolean inDefault, boolean isAsync, boolean allowGenerator) throws JsError {
        this.lex();
    ...
    @Nonnull
    protected Expression parseFunctionExpression(boolean isAsync, boolean allowGenerator) throws JsError {
        AdditionalStateT startState = this.startNode();
        this.lex();
    ...
    protected BindingBindingWithDefault parseBindingElement() throws JsError {
        ...
        if (this.eat(TokenType.ASSIGN)) {
            ...
            boolean previousAwaitExpression = this.allowAwaitExpression;
            ...
            this.allowAwaitExpression = previousAwaitExpression;
        }
        return bbwd;
    }
        ...
        switch (this.lookahead.type) {
            ...
            default: {
                ...
                if (expr instanceof IdentifierExpression && this.eat(TokenType.COLON)) {
                    Statement labeledBody = this.match(TokenType.FUNCTION) ? this.parseFunctionDeclaration(this.startNode(), false, false, false) : this.parseStatement();
    ...
    protected Statement parseContinueStatement() throws JsError {
        ...
        if (this.match(TokenType.IDENTIFIER) || this.match(TokenType.AWAIT) || this.match(TokenType.YIELD) || this.match(TokenType.LET)) {
    ...
    protected Either<Expression, AssignmentTarget> parseAssignmentExpressionOrTarget() throws JsError {
        ...
        if (this.allowAwaitExpression && this.match(TokenType.AWAIT)) {
            this.isBindingElement = this.isAssignmentTarget = false;
            return Either.left(this.parseAwaitExpression());
        }
        ...
        if (this.match(TokenType.IDENTIFIER) || this.match(TokenType.AWAIT) || this.match(TokenType.YIELD) || this.match(TokenType.LET)) {
            ...
            if (!this.hasLineTerminatorBeforeNext && this.match(TokenType.ARROW)) {
                ...
                return Either.left(this.parseArrowExpressionTail(isAsync, this.finishArrowParams(params, Maybe.empty(), startState), startState));
            }
        } else {
            expr = this.parseConditionalExpression();
        }
    ...
    @Nonnull
    protected Expression parseAwaitExpression() throws JsError {
        AdditionalStateT startState = this.startNode();
        this.lex();

        Maybe<Expression> expr = Maybe.empty();
        if (this.lookaheadAssignmentExpression()) {
            expr = this.parseAssignmentExpression().left();
        }

        return this.finishNode(startState, new AwaitExpression(expr.fromJust()));
    }
    ...
    @Nonnull
    protected Either<Expression, AssignmentTarget> parseGroupExpression(AdditionalStateT preParenStartState, boolean isAsync) throws JsError {
        SourceLocation startLocation = this.getLocation();
        this.allowAwaitExpression = isAsync;
        this.allowYieldExpression = !isAsync;

        this.expect(TokenType.LPAREN);
        AdditionalStateT postParenStartState = this.startNode();

        if (this.match(TokenType.RPAREN)) {
            FormalParameters paramsNode = this.finishArrowParams(new ArrayList<>(), Maybe.empty(), postParenStartState);
            this.lex();
            this.isBindingElement = this.isAssignmentTarget = false;
            return Either.left(this.parseArrowExpressionTail(isAsync, paramsNode, preParenStartState));
        } else if (this.eat(TokenType.ELLIPSIS)) {
            Maybe<Binding> rest = Maybe.of(this.parseBindingTarget());
            FormalParameters paramsNode = this.finishArrowParams(new ArrayList<>(), rest, postParenStartState);
            this.expect(TokenType.RPAREN);
            this.isBindingElement = this.isAssignmentTarget = false;
            return Either.left(this.parseArrowExpressionTail(isAsync, paramsNode, preParenStartState));
        }
        ...
        while (this.eat(TokenType.COMMA)) {
            ...
            if (this.match(TokenType.ELLIPSIS)) {
                ...
                this.lex();
                Maybe<Binding> rest = Maybe.of(this.parseBindingTarget());
                FormalParameters paramsNode = this.finishArrowParams(params, rest, postParenStartState);
                this.expect(TokenType.RPAREN);
                return Either.left(this.parseArrowExpressionTail(isAsync, paramsNode, preParenStartState));
            }
        ...
        FormalParameters paramsNode = this.finishArrowParams(params, Maybe.empty(), postParenStartState);
        this.expect(TokenType.RPAREN);
    ...
    protected Either<ObjectProperty, AssignmentTargetProperty> parsePropertyDefinition() throws JsError {
        ...
        if (keyOrMethod.isRight()) {
            ...
            if (propName instanceof StaticPropertyName) {
                ...
                if (!this.match(TokenType.COLON)) {
                    if (token.type != TokenType.IDENTIFIER && token.type != TokenType.AWAIT  && token.type != TokenType.YIELD && token.type != TokenType.LET) {
                        throw this.createUnexpected(token);
                    }
    ...
    protected Either<PropertyName, MethodDefinition> parseMethodDefinition() throws JsError {
        ...
        if (this.match(TokenType.LPAREN)) {
            ...
            return Either.right(this.finishNode(startState, new Method(isAsync, isGenerator, name, params, body)));
        }
        ...
        if (isAsync && this.match(TokenType.COLON)) {
            throw this.createUnexpected(this.lookahead);
        }
    ...
    protected Expression parseClass() throws JsError {
        ...
        boolean previousParamAwait = this.allowAwaitExpression;
        this.allowYieldExpression = false;
        this.allowAwaitExpression = false;
        ...
        this.allowAwaitExpression = previousParamAwait;
    ...
    protected ExportDeclaration parseExportDeclaration() throws JsError {
        ...
        switch (this.lookahead.type) {
            ...
            case CLASS:
                ...
                break;
            case ASYNC:
                decl = new Export((FunctionDeclaration) this.parseAsync(false));
                break;
            case FUNCTION:
                decl = new Export((FunctionDeclaration) this.parseFunctionDeclaration(this.startNode(), false, false, true));
                break;
            case DEFAULT:
                this.lex();
                switch (this.lookahead.type) {
                    case ASYNC:
                        decl = new ExportDefault((FunctionDeclaration) this.parseAsync(true));
                        break;
                    case FUNCTION:
                        decl = new ExportDefault((FunctionDeclaration) this.parseFunctionDeclaration(this.startNode(), true, false, true));
                        break;
                    case CLASS:
