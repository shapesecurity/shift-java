...
import static com.shapesecurity.shift.es2018.parser.ParserTestCase.testModule;
...
public class AsyncAwaitTest {
	...
	@Test
	public void testAsyncGenerator() throws JsError {
		testScript("async function *a(){}", new FunctionDeclaration(true, true, new BindingIdentifier("a"), new FormalParameters(ImmutableList.empty(), Maybe.empty()), new FunctionBody(ImmutableList.empty(), ImmutableList.empty())));
		testScript("(async function *a(){})", new FunctionExpression(true, true, Maybe.of(new BindingIdentifier("a")), new FormalParameters(ImmutableList.empty(), Maybe.empty()), new FunctionBody(ImmutableList.empty(), ImmutableList.empty())));
		testScript("async function *a() { function b(c = await (0)) {} }", new FunctionDeclaration(true, true, new BindingIdentifier("a"), new FormalParameters(ImmutableList.empty(), Maybe.empty()),
				new FunctionBody(ImmutableList.empty(), ImmutableList.of(
						new FunctionDeclaration(false, false,
								new BindingIdentifier("b"),
								new FormalParameters(ImmutableList.of(
		...
		testScript("(async function*() { function b(c = await (0)) {} })", new FunctionExpression(true, true, Maybe.empty(), new FormalParameters(ImmutableList.empty(), Maybe.empty()),
				new FunctionBody(ImmutableList.empty(), ImmutableList.of(
						new FunctionDeclaration(false, false,
								new BindingIdentifier("b"),
								new FormalParameters(ImmutableList.of(
		...
		testScript("({ async *a(){} })", new ObjectExpression(ImmutableList.of(new Method(true, true, new StaticPropertyName("a"), new FormalParameters(ImmutableList.empty(), Maybe.empty()), new FunctionBody(ImmutableList.empty(), ImmutableList.empty())))));
		testScript("({ async *get(){} })", new ObjectExpression(ImmutableList.of(new Method(true, true, new StaticPropertyName("get"), new FormalParameters(ImmutableList.empty(), Maybe.empty()), new FunctionBody(ImmutableList.empty(), ImmutableList.empty())))));
		testScript("(class { async *a(){} })", new ClassExpression(Maybe.empty(), Maybe.empty(), ImmutableList.of(new ClassElement(false, new Method(true, true, new StaticPropertyName("a"), new FormalParameters(ImmutableList.empty(), Maybe.empty()), new FunctionBody(ImmutableList.empty(), ImmutableList.empty()))))));
		testScript("(class { static async *a(){} })", new ClassExpression(Maybe.empty(), Maybe.empty(), ImmutableList.of(new ClassElement(true, new Method(true, true, new StaticPropertyName("a"), new FormalParameters(ImmutableList.empty(), Maybe.empty()), new FunctionBody(ImmutableList.empty(), ImmutableList.empty()))))));
		testModule("export async function *a(){}", new Export(new FunctionDeclaration(true, true, new BindingIdentifier("a"), new FormalParameters(ImmutableList.empty(), Maybe.empty()), new FunctionBody(ImmutableList.empty(), ImmutableList.empty()))));
		testModule("export default async function* (){}", new ExportDefault(new FunctionDeclaration(true, true, new BindingIdentifier("*default*"), new FormalParameters(ImmutableList.empty(), Maybe.empty()), new FunctionBody(ImmutableList.empty(), ImmutableList.empty()))));
	...
	public void testAsyncFailure() {
		...
		testScriptFailure("function* a(){ async (yield) => {}; }", 29, String.format(ErrorMessages.UNEXPECTED_TOKEN, "=>"));
		testScriptFailure("async await => 0", 6, "\"await\" may not be used as an identifier in this context");
		testScriptFailure("async (await) => 0", 7, ErrorMessages.NO_AWAIT_IN_ASYNC_PARAMS);
		...
		testScriptFailure("async (a = await => {}) => {}", 11, ErrorMessages.NO_AWAIT_IN_ASYNC_PARAMS);
		testScriptFailure("async (a = (await) => {}) => {}", 12, ErrorMessages.NO_AWAIT_IN_ASYNC_PARAMS);
		testScriptFailure("async (a = aw\\u{61}it => {}) => {}", 11, ErrorMessages.NO_AWAIT_IN_ASYNC_PARAMS);
		testScriptFailure("async (a = (b = await (0)) => {}) => {}", 16, ErrorMessages.NO_AWAIT_IN_ASYNC_PARAMS);
