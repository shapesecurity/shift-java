...
public class ObjectBindingTest extends ParserTestCase {
    ...
    public void testObjectBinding() throws JsError {
        ...
        testScript("({a:let} = 0);", new AssignmentExpression(new ObjectBinding(ImmutableList.list(new BindingPropertyProperty(new StaticPropertyName("a"), new BindingIdentifier("let")))), new LiteralNumericExpression(0.0)));
        testScript("({let} = 0);", new AssignmentExpression(new ObjectBinding(ImmutableList.list(new BindingPropertyIdentifier(new BindingIdentifier("let"), Maybe.nothing()))), new LiteralNumericExpression(0.0)));
