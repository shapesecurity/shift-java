...
public class DirectiveTest extends ParserTestCase {
    ...
    public void testDirective() throws JsError {
        ...
        testScript("(function () { 'use\\x20strict'; with (i); })", new FunctionExpression(false, Maybe.nothing(), new FormalParameters(ImmutableList.nil(), Maybe.nothing()), new FunctionBody(ImmutableList.list(new Directive("use\\x20strict")), ImmutableList.list(new WithStatement(new IdentifierExpression("i"), new EmptyStatement())))));
        testScript("(function () { 'use\\nstrict'; with (i); })", new FunctionExpression(false, Maybe.nothing(), new FormalParameters(ImmutableList.nil(), Maybe.nothing()), new FunctionBody(ImmutableList.list(new Directive("use\\nstrict")), ImmutableList.list(new WithStatement(new IdentifierExpression("i"), new EmptyStatement())))));
        testScript("function a() {'use strict';return 0;};", new FunctionDeclaration(false, new BindingIdentifier("a"), new FormalParameters(ImmutableList.nil(), Maybe.nothing()), new FunctionBody(ImmutableList.list(new Directive("use strict")), ImmutableList.list(new ReturnStatement(Maybe.just(new LiteralNumericExpression(0.0)))))));
        testScript("(function() {'use strict';return 0;});", new FunctionExpression(false, Maybe.nothing(), new FormalParameters(ImmutableList.nil(), Maybe.nothing()), new FunctionBody(ImmutableList.list(new Directive("use strict")), ImmutableList.list(new ReturnStatement(Maybe.just(new LiteralNumericExpression(0.0)))))));
