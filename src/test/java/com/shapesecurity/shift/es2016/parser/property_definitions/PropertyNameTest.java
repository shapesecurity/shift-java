...
public class PropertyNameTest extends ParserTestCase {
    ...
    public void testPropertyName() throws JsError {
        ...
        testScript("({[6+3]() {}})", new ObjectExpression(ImmutableList.of(new Method(false, new ComputedPropertyName(new BinaryExpression(new LiteralNumericExpression(6.0), BinaryOperator.Plus, new LiteralNumericExpression(3.0))), new FormalParameters(ImmutableList.empty(), Maybe.empty(), false), new FunctionBody(ImmutableList.empty(), ImmutableList.empty())))));
        testScript("({3() {}})", new ObjectExpression(ImmutableList.of(new Method(false, new StaticPropertyName("3"), new FormalParameters(ImmutableList.empty(), Maybe.empty(), false), new FunctionBody(ImmutableList.empty(), ImmutableList.empty())))));
        testScript("({\"moo\"() {}})", new ObjectExpression(ImmutableList.of(new Method(false, new StaticPropertyName("moo"), new FormalParameters(ImmutableList.empty(), Maybe.empty(), false), new FunctionBody(ImmutableList.empty(), ImmutableList.empty())))));
        testScript("({\"oink\"(that, little, piggy) {}})", new ObjectExpression(ImmutableList.of(new Method(false, new StaticPropertyName("oink"), new FormalParameters(ImmutableList.of(new BindingIdentifier("that"), new BindingIdentifier("little"), new BindingIdentifier("piggy")), Maybe.empty(), false), new FunctionBody(ImmutableList.empty(), ImmutableList.empty())))));
